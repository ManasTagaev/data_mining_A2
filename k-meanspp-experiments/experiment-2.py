...
def plot(ax, centroids, title, time_spent):
    ...
    for i in range(num_clusters):
        ...
        ax.scatter(cluster_data[:, 0], cluster_data[:, 1],
                   marker='.', color=colors[i], label=f'Cluster {i}')
    ...
    padding = 10000
    ax.set_xlim(all_points[:, 0].min() - padding,
                all_points[:, 0].max() + padding)
    ax.set_ylim(all_points[:, 1].min() - padding,
                all_points[:, 1].max() + padding)

    # Add text annotation for time spent
    ax.text(0.98, 0.97, f'Time: {time_spent:.4f} s', transform=ax.transAxes,
...
def initialize(data, k, ax):
    ...
    spent = end - start
    print("Time needed: ", spent)
    plot(ax, np.array(centroids), 'k-means++ Initialization', spent)
    return centroids
